...
// Decode a json string to a struct and a map
func (c *Context) BindToMapAndStruct(structContainer interface{}, mapContainer *map[string]interface{}) error {

	var str = ""

	util.ReadReader(c.Request().Body(), 1024, func(err error, bs []byte, done bool) bool {
		if !done {
			str += string(bs)
		}
		return true
	...
	err := json.Unmarshal([]byte(str), structContainer)
	if err != nil {
		return err
	}

	err = json.Unmarshal([]byte(str), mapContainer)
	if err != nil {
		return err
	}
...
// Get route param and convert to uint
func (c *Context) ParamUInt(key string) uint {
	return uint(util.ToInt64(c.Param(key)))
}
...
func (c *Context) GetUInt(key string) uint {
	return uint(util.ToInt64(c.Get(key)))
}
